...
function clearQuestTimer(questId) {
  ...
  if (timer) { clearTimeout(timer); activeQuestTimers.delete(questId); }
}

// ── Quest Lifecycle ───────────────────────────────────────────────────────────
async function activateNextQuest(requestId) {
  const nextQuestResult = await pool.query(
    `SELECT q.id, q.request_id, q.donor_id, q.distance_meters, q.expires_at,
            u.name, u.blood_type, u.device_token,
  ...
  if (!nextQuest) {
    // No more donors available — keep request in 'matching' state
    await pool.query(
      `UPDATE blood_requests SET status = 'matching', updated_at = NOW() WHERE id = $1`,
      [requestId]
    );
    return null;
  }

  await pool.query(`UPDATE quests SET notified_at = NOW() WHERE id = $1`, [nextQuest.id]);
  await pool.query(
    `UPDATE blood_requests SET status = 'notified', updated_at = NOW() WHERE id = $1`,
    [requestId]
  );
  ...
  await logNotification({
    ...
    body: `Blood needed at ${nextQuest.hospital_name}. Tap to respond.`,
    data: {
  ...
  const timer = setTimeout(() => {
    handleQuestExpiry(nextQuest.id).catch((err) =>
      console.error('Quest expiry failed:', err)
    );
  }, QUEST_EXPIRY_MS);
...
async function handleQuestExpiry(questId) {
  ...
  const questResult = await pool.query(
    `SELECT id, request_id, status FROM quests WHERE id = $1`,
    [questId]
  );
  const quest = questResult.rows[0];
  if (!quest || quest.status !== 'pending') return null;

  await pool.query(
    `UPDATE quests SET status = 'expired', responded_at = NOW() WHERE id = $1`,
    [questId]
  );
...
// ── Donor Matching ────────────────────────────────────────────────────────────
async function findCompatibleDonors(hospitalId, bloodType) {
  const compatibleDonorTypes = getCompatibleDonorTypes(bloodType);
  if (!compatibleDonorTypes.length) return [];

  const result = await pool.query(
    `SELECT
       u.id, u.name, u.email, u.blood_type, u.device_token,
       CASE
         WHEN u.location IS NOT NULL
         THEN ROUND(ST_Distance(u.location, h.location))::integer
         ELSE NULL
     ...
     LIMIT 5`,
    [hospitalId, compatibleDonorTypes]
  );
  return result.rows;
}

async function createRequestWithQuests({ requesterId, hospitalId, bloodType, unitsNeeded, urgency, notes }) {
...
async function getRequestDetails(requestId) {
  ...
  return { request, quests: questsResult.rows };
}
...
async function acceptQuest(questId, donorId) {
  ...
  await pool.query(
    `UPDATE quests SET status = 'accepted', responded_at = NOW() WHERE id = $1`,
    [questId]
  );
  await pool.query(
    `UPDATE blood_requests SET status = 'accepted', updated_at = NOW() WHERE id = $1`,
    [quest.request_id]
  );

  return { status: 200, body: { questId } };
}
...
async function declineQuest(questId, donorId) {
  ...
  clearQuestTimer(questId);
  await pool.query(
    `UPDATE quests SET status = 'declined', responded_at = NOW() WHERE id = $1`,
    [questId]
  );
...
app.post('/auth/register', async (req, res) => {
  try {
    ...
    if (!['donor', 'requester'].includes(role)) {
      return res.status(400).json({ error: 'Invalid role. Must be donor or requester.' });
    }
    ...
    const result = await pool.query(
      `INSERT INTO users (name, email, phone, password_hash, role, blood_type, location)
       VALUES ($1, $2, $3, $4, $5, $6, ${locationQuery})
  ...
  try {
    const result = await pool.query(
      `SELECT id, name, email, phone, role, blood_type,
              ...
              is_available, cooldown_until, xp, level, donation_count
...
app.post('/requests', authenticateToken, async (req, res) => {
  try {
    const { hospital_id, blood_type, units_needed, urgency, notes } = req.body;
...
app.get('/quests/active', authenticateToken, async (req, res) => {
  try {
    ...
    if (!quest) return res.status(404).json({ error: 'No active quest found' });
    res.json({ quest });
  } catch (error) {
    console.error(error);
...
app.get('/quests/history', authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT q.*, br.blood_type as request_blood_type, br.urgency, br.units_needed,
              h.name AS hospital_name, h.address AS hospital_address
       FROM quests q
       JOIN blood_requests br ON br.id = q.request_id
       JOIN hospitals h ON h.id = br.hospital_id
...
app.post('/checkin/simulate', authenticateToken, async (req, res) => {
  try {
    ...
    const questResult = await pool.query(
      `SELECT q.*, br.urgency FROM quests q
       JOIN blood_requests br ON br.id = q.request_id
       WHERE q.id = $1`,
      [quest_id]
    );
    ...
    if (quest.donor_id !== req.user.id) return res.status(403).json({ error: 'This quest belongs to another donor' });
    if (quest.status !== 'accepted') return res.status(400).json({ error: 'Quest must be accepted to check in' });
    ...
    await pool.query(
      `UPDATE users SET xp = $1, level = $2, donation_count = $3 WHERE id = $4`,
      [newXp, newLevel, newDonationCount, req.user.id]
    );
...
app.listen(PORT, () => {
  console.log(`RedQuest API running on port ${PORT}`);
});
